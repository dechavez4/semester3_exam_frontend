...
const People = () => {
  ...
  const [mySelf, setMyself] = useState({email: "", id: "", name: "", signups: []});

  useEffect(() => {
    facade.fetchData().then(res => setDataFromServer(res.msg));
    console.log(facade.getTokenInfo().username);
    facade.fetchMyself(facade.getTokenInfo().username).then(res => {setMyself(... mySelf, res); console.log(mySelf);});
  }, []);
  useEffect(() => {
    let didCancel = false;
...
const Edit = () => {
  ...
  const emptyCourse = { courseName: "", description: ""};
  const [FindCourse, setFindCourse] = useState(emptyCourse);
    ...
    <div className="container-fluid">
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          <h4>add Class</h4>
          <input className="col-md-4" type="text" name="SemesterNname" placeholder="Type semester name"></input><br/>
